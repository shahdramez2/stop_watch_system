...
#define Comparematch 15625  /* number used in the compare register of Timer 1 */
#define MINUTES_SEC_MAX_NUM 60
#define HOURS_MAX_NUM 12
...
int main (void)
{
	...
	for( ; ; )
	{
		...
		if(MINUTES_SEC_MAX_NUM == g_StopWatch.seconds)
		{
			g_StopWatch.minutes++;
			g_StopWatch.seconds=0;
		}

		if(MINUTES_SEC_MAX_NUM == g_StopWatch.minutes)
		{
			g_StopWatch.hours++;
			g_StopWatch.minutes=0;
		}

		if(HOURS_MAX_NUM == g_StopWatch.hours)
		{
			g_StopWatch.seconds=0;
			g_StopWatch.minutes=0;
			g_StopWatch.hours=0;
